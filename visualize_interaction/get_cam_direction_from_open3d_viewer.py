...
# filename = "../data/model/0/model_duck.ply"  # 模型文件
# filedir = "../data/model/0/cam_dirs"
filename = "../data/model/1/model_pot.ply"  # 模型文件
filedir = "../data/model/1/cam_dirs"
...
from process_data.point_cloud_preprocess import prepare_model_and_open3d_visualize
...
def save_cam_dir_from_json(out_filename):
    ...
    try:
        ...
        estimator = o3d.run_registration.TransformationEstimationPointToPoint()
...
def get_cam_directions():
    del_file_in_folder(filedir)
    for i in range(100):
        result = save_cam_dir_from_json(filedir+"/dir_%s.txt" % i)
        if result != 0:
            print("读取json时错误")
